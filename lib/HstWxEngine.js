...
class HstWxEngine {
  constructor(options) {
    ...
    self.ws = new Websocket({
      url: options || 'wss://fsp-wxgw1.hst.com:50001',
  ...
  /**
   * 当服务器通知其他小程序端广播媒体消息时(14109) 本地小程序应该请求音视频
   */
  _subscribeMulti(groupID, userID, userArr) {
    ...
    userArr.forEach(user_info => {
      ...
      mediaArr.forEach(media_info => {
        ...
        let info = media_info.media_id + '_' + type + '_' + user_info.user_id
  ...
  _subscribeOne(groupID, userID, mediaInfo) {
    ...
    let info = mediaInfo.media_id + '_' + type + '_' + mediaInfo.user_id
