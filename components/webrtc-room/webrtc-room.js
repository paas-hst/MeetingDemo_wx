...
Component({
  ...
  data: {
    ...
    creator: '',
    playerMutedStatus: {}, // 记录live-player的声音状态， 默认都是打开声音状态(false和undefined)
    playerVideoStatus: {}, // 记录live-player的摄像头状态， 默认都是打开声音状态（true和undefined）
    ...
    paasPubUserList: [], // paas广播了媒体的用户列表
  },
  ...
  methods: {
    ...
    initLayout(templateName) {
      ...
      switch (templateName) {
        case CONSTANT.TEMPLATE_TYPE['1PLAYER']:
          this.setData({
            maxMembers: 1,
            members: [{}],
            presenter: [],
          ...
          break;
        case CONSTANT.TEMPLATE_TYPE['1PUSHER']:
          this.setData({
            maxMembers: 1,
            members: [{}],
            presenter: [],
        ...
        case CONSTANT.TEMPLATE_TYPE['1V1']:
          ...
          break;
        case CONSTANT.TEMPLATE_TYPE['GRID4']:
          this.setData({
            maxMembers: 4,
            members: [{}, {}, {}],
            presenter: [{}],
          ...
          break;
        case CONSTANT.TEMPLATE_TYPE['GRID6']:
          this.setData({
            maxMembers: 6,
            members: [{}, {}, {}, {}, {}],
            presenter: [{}],
    ...
    /**
     * 刷新微信小程序用户的广播状态，默认情况，小程序端加入房间都会出现
     * 在pushlist中，但在paas场景，入会并不会自动广播音视频
     */
    updatePaasPubUserList(pubList) {
      this.setData({paasPubUserList: pubList});
    },
     ...
     */
    isRealPusher(userId) {
      let pattern = new RegExp('^' + this.data.roomID + '_')
      ...
      // 能够匹配说明确实广播了媒体
      for (const user of this.data.paasPubUserList) {
        if (userId == this.data.roomID + '_' + user) {
          return true
        }
      }
    ...
    enableVideo(enable, userid) {
      ...
      if (!playerContext) {
        console.error("Create player context failed!")
        return
      }
     ...
     */
    enableVidoTap(e) {
      console.warn("enableVidoTap: " + e.currentTarget.dataset.uid)
      var uid = e.currentTarget.dataset.uid;
    ...
    onWebRTCUserListPush: function (msg) {
      ...
      newUserList && newUserList.forEach(function (val) {
        if (self.isRealPusher(val.userid)) {
          var pusher = {
            userID: val.userid,
      ...
      if (pushers.length > self.data.maxMembers) {
        self.postErrorEvent(
          self.data.ERROR_EXCEEDS_THE_MAX_MEMBER, 
          `当前房间超过最大人数${self.data.maxMembers + 1}，请重新进入其他房间体验~`);
      }

      self.onPusherJoin({pushers: pushers});
      self.onPusherQuit({pushers: pushers});
    },
    ...
    onPusherJoin: function (res) {
      res.pushers.forEach(function (val) {
        ...
        if (!hasPlay && emptyIndex != -1) {
          ...
          console.warn("Create live player context, userID: " + val.userID);
        }
    ...
    onPush: function (e) {
      ...
      var code = e.detail ? e.detail.code : e;
      var errmessage = '';
      switch (code) {
        case 1002: {
          console.log('推流成功');
